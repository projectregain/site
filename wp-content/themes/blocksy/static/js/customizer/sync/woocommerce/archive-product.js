...
export const replaceCards = () => {
  ...
  ;[...cache.querySelectorAll('[data-product-id]')].map(el => {
    const product = getFreshHtmlFor(el.dataset.productId, cache, 'product-id')
    ...
    while (product.firstElementChild) {
      ...
      const article = document.querySelector('.shop-entries').lastElementChild
        ...
        if (article.querySelector('.price')) {
          article
            ...
            .insertBefore(
              ...
              article.querySelector('.woo-card-actions').firstElementChild
            )
        }

        if (article.querySelector('.woocommerce-loop-product__title')) {
          article.appendChild(
            article.querySelector('.woocommerce-loop-product__title')
          )
        }

        if (article.querySelector('.product-categories')) {
          article.appendChild(article.querySelector('.product-categories'))
        }

        if (article.querySelector('.woo-card-actions')) {
          article.appendChild(article.querySelector('.woo-card-actions'))
        }
      ...
      if (article.querySelector('.star-rating')) {
        if (wp.customize('has_star_rating')() === 'no') {
          article
            ...
            .parentNode.removeChild(article.querySelector('.star-rating'))
        }
      }
      ...
      if (article.querySelector('.onsale')) {
        if (wp.customize('has_sale_badge')() === 'no') {
          article
            ...
            .parentNode.removeChild(article.querySelector('.onsale'))
        }
      }
