      ...
      modifiers: {
        ...
        setCustomStyle: {
          ...
          fn: ({
            ...
            (popper.dataset.placement = placement === 'left' ? 'left' : 'right')
        }
      },
      placement: 'right'
    }
...
export const mount = () => {
  if (!$) return
  ;[
    ...document.querySelectorAll('.ct-header-cart:not([data-skip-dropdown])')
  ].map(singleCart => {
  ...
  $(document.body).on('added_to_cart', (_, fragments, __, button, quantity) => {
    button = button[0]
    ;[...document.querySelectorAll(selector)].map(cart => {
      ...
      if (document.querySelector('.ct-cart-content')) {
        cart.querySelector('.ct-cart-content').innerHTML = Object.values(
          fragments
        )[0]
      }
  ...
  $(document.body).on('removed_from_cart', (_, __, ___, button) =>
    [...document.querySelectorAll(selector)].map(cart => {
      ...
      cart.querySelector('[data-count]').dataset.count =
        parseInt(cart.querySelector('[data-count]').dataset.count, 10) -
        parseInt(
          button[0].closest('li').querySelector('span.quantity').textContent,
          10
        )
    })
  )
